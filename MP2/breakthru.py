#!/usr/bin/python
from gamestate import BoardState
import sys
import time
import math
...
def eval_fn(state,p_color,o_rat,d_rat):
	...
	p_col = [1 for i in range(8)]
	op_col = [1 for i in range(8)]

	p_col_num = [0 for i in range(8)]
	p_row_num = [0 for i in range(8)]

	op_col_num = [0 for i in range(8)]
	op_row_num = [0 for i in range(8)]
...
print "Nodes Expanded per Turn: "+str((black_states+white_states)/(len(game)-1))
...
fo = open('breakthru_'+str(option)+'_'+str(depth)+'.txt','w')
fo.write(str_out)
fo.close()
